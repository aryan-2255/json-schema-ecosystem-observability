...
These two numbers measure very different things. Reading them together is where the real signal is.
...
The ratio between these two numbers — roughly 100,000:1 — represents an enormous unreached audience. If even a fraction of the teams silently depending on `ajv` could be drawn into the ecosystem as contributors, documentation readers, or RFC commenters, the community would look fundamentally different. Observability work that makes this invisible adoption visible — tracking who is downloading, which validator versions, what tooling they combine it with — is the groundwork for reaching that audience.

**Combined, the metrics represent two distinct layers:**

| Layer | Metric | What it reflects |
|---|---|---|
| Production adoption | npm weekly downloads | How embedded JSON Schema is in shared infrastructure |
| Community engagement | GitHub topic repos | How many developers explicitly identify with the ecosystem |

A healthy ecosystem needs both to grow together. Right now, the infrastructure layer is enormous and the community engagement layer is comparatively small. That is not a failure — it is a growth opportunity that observability data can help prioritize.
...
Being honest about what a metric does *not* measure is as important as knowing what it does. These two signals have real blind spots.
